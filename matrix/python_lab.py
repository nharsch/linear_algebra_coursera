...
X1 = { 2, 3, 4}
Y1 = { 5, 6, 7}
assert len({x*y for x in X1 for y in Y1}) == 9
...
assert S_intersect_T == S.intersection(T)
#
#
#
# ## 10: (Task 10) Average
...
assert len(cartesian_product) == 9
#
#
#
...
assert (0,0,0) not in exclude_zero_list and set(zero_sum_list)- set(exclude_zero_list) == {(0,0,0)}
#
#
#
...
first_of_tuples_list = next((i,j,k) for i in S for j in S for k in S if i+j+k == 0 and (i,j,k) != (0,0,0))
...
L = ['A','B','C','D','E']
range_and_zip = list(zip(L,range(len(L))))
#
#
#
...
list_sum_zip = [sum(e) for e in zip(A,B)]
#
#
#
...
representation_dict = {n:tuple('{:03d}'.format(n)) for n in range(1000)}
#
#
#
...
listdict2dict = {k:v for k,v in zip(names, id2salary.values())}
#
#
#
